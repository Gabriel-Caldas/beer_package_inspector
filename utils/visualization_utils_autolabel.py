...
def visualize_boxes_and_labels_on_image_array(
            ...
            if scores is None:
                ...
                if not skip_labels:
                    if not agnostic_mode:
                        ...
                        objs_classes.append(class_name)
                if not skip_scores:
                    if not display_str:
    ...
    #print("BOX_TO_COLOR_MAP ", len(box_to_color_map))
    # Draw all boxes onto image.
    for box, color in box_to_color_map.items():
        ymin, xmin, ymax, xmax = box
        # image.shape = (height, width, channels)
        heigth, width = image.shape[0:2]
        denormalized_box = ymin*heigth, xmin*width, ymax*heigth, xmax*width
        # list to save the detections dimensions
        objs_detec_dims.append(denormalized_box)
        if instance_masks is not None:
    ...
    detec_data = {'classes':objs_classes, 'coordinates': objs_detec_dims}


    return image, detec_data
