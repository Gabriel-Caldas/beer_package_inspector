...
def save_classified_image(classified_img, index):
    cv2.imshow('Classified_Frame', classified_img)
    cv2.imwrite('save_test/img{}.jpg'.format(index), classified_img)
    index += 1
...
def visualize_boxes_and_labels_on_image_array(
    ...
    # Save images with boxes on disk
    if to_save == True and class_name != 'Good_Pack' and time.time()-t >= 1:
        t = time.time()
        classified_img_index = save_classified_image(image, classified_img_index)
        if class_name == 'Bad_Shape':
            bs += 1
        else:
            bh += 1
        os.system('clear')
        print("PACOTES BICUDOS: ", bs)
        print("PACOTES FURADOS: ", bh)



    return image,classified_img_index
